...
# 課題6
Q1. MySQL診断ツール (MySQL Tuner)をした結果、以下のように表示されました。query_cache_sizeに付いて説明してください。
```
-------- Recommendations ---------------------------------------------------------------------------
// 省略
query_cache_size (>= 8M)
```
Q2. 【復習】カバリングインデックスとはなんですか？
Q3. データベースのスケーリング: 垂直スケーリングと水平スケーリングそれぞれ説明してください。



A1. クエリーのキャッシュサイズのこと。診断結果は、現状キャッシュが無効になっており、有効にすることでパフォーマンスが改善するかもよというもの。ただし、高負荷の場合、かつクエリキャッシュヒット率が低い場合、設定をOFFにしたほうがいい場合もあります。
[MySQLパフォーマンスチューニング -my.cnfの見直し-](https://qiita.com/mamy1326/items/9c5eaee3c986cff65a55#-query_cache_size--8m)

A2. クエリを処理するのに必要なデータを全て含んでいるようなインデックスをカバリングインデックスと呼ぶ。
A3. 
垂直スケーリング: 既存のサーバーのCPUやRAM、ストレージといったリソースを物理的に拡張・縮小する方法です。
水平スケーリング: 複数のサーバーにデータベースの負荷を分散させる方法です。

[データベーススケーリングの基本：垂直と水平スケーリングについて解説](https://products.sint.co.jp/siob/blog/scaling)
...
CSRは、固定の雛形になるHTMLにデータを詰めるため、build時にHTMLが生成される。
...
- 週1回更新されるブログ
  - SSG
  - 更新頻度が低いため、SSGとして運用するコストが低いことと、サーバーを低コストで運用できる。
- ユーザーのコメントが随時追加されるクックパッドのようなサービス
  - SSRかCSR
  - 初回ロードの速度とSEOを重視するならSSR。スケーラビリティやユーザーがサイト内を回遊する際の快適性を重視するならCSR。
- freeeのような会計サービス
  - CSR
  - Webアプリケーションのように機能をリッチにしたいなら、CSRにしてクライアントを賢くしてあげるのが良さげ。
- 経営指標（OKRやKPIなど）を管理する社内サービス
  - CSR
  - SEOを気にする必要がないのと、ページを回遊する際の快適性が重要になるため。
- 社内SNS
  - CSR
  - 更新頻度が高く、動的なデータ更新が多く求められるため。